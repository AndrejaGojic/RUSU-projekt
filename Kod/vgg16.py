...
data = np.concatenate([Hard_data], axis=0)
...
history = vgg.fit(train_data, train_labels, batch_size = 128, epochs = 15, 
...
test_images_resize = np.array(test_images).reshape(-1, img_size, img_size, 3)
...
res = vgg.evaluate(test_images_resize, test_labels)
print("    Test Loss: {:.5f}".format(res[0]))
print("Test Accuracy: {:.2f}%".format(res[1] * 100))
pred = vgg.predict(test_images_resize)
